...
useEffect(() => {
  async function fetchReservations() {
    const response = await fetch("http://192.168.183.29:5000/api/reservations");
...
useEffect(() => {
  async function fetchListings() {
    const response = await fetch("http://192.168.183.29:5000/api/listings");
    const data = await response.json();
    setListings(data);
  }
  fetchListings();
...
// Function to open AddStays form
  return (
...
<div className="absolute right-4 flex items-center">
	<Link to={'/addStays'}
className="sm:hidden p-2 rounded-md text-blue-600 hover:bg-gray-100 min-h-[44px] min-w-[44px]"
	aria-label="Add listings">
<Plus size={26} />
	</Link>

<button className="sm:hidden p-2 rounded-md text-blue-600 hover:bg-gray-100 min-h-[44px] min-w-[44px]"
 aria-label="Settings">
<Settings size={24} />
        </button>
</div>
